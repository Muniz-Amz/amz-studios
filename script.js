...
function campoCrachaPrivadoId(serverId, campo) {
    return `admin-private-badge-${serverId}-${campo}`;
}

function valorCampoCrachaPrivado(serverId, campo) {
    return document.getElementById(campoCrachaPrivadoId(serverId, campo))?.value?.trim() || '';
}

function setValorCampoCrachaPrivado(serverId, campo, valor) {
    const elemento = document.getElementById(campoCrachaPrivadoId(serverId, campo));
    if (elemento) elemento.value = valor ?? '';
}

function renderizarPainelPrivadoGuildAdmin(servidor) {
    ...
    return `
        <details class="admin-details admin-private-details">
            ...
            <div class="admin-private-panel" data-private-server-id="${escaparHTML(serverId)}">
                ...
                <div class="admin-private-layout">
                    <form class="admin-private-form" onsubmit="salvarCrachaPrivadoAdmin(event, '${escaparHTML(serverId)}')">
                        <input type="hidden" id="${escaparHTML(campoCrachaPrivadoId(serverId, 'id'))}">
                        <label>
                            <span>Nome no crachá</span>
                            <input id="${escaparHTML(campoCrachaPrivadoId(serverId, 'memberName'))}" type="text" placeholder="Nome do membro" oninput="atualizarPreviewCrachaPrivadoAdmin('${escaparHTML(serverId)}')">
                        </label>
                        <label>
                            <span>ID Discord</span>
                            <input id="${escaparHTML(campoCrachaPrivadoId(serverId, 'memberId'))}" type="text" placeholder="ID do usuário" oninput="atualizarPreviewCrachaPrivadoAdmin('${escaparHTML(serverId)}')">
                        </label>
                        <label>
                            <span>Título</span>
                            <input id="${escaparHTML(campoCrachaPrivadoId(serverId, 'badgeTitle'))}" type="text" placeholder="Crachá AMZ" oninput="atualizarPreviewCrachaPrivadoAdmin('${escaparHTML(serverId)}')">
                        </label>
                        <label>
                            <span>Rank</span>
                            <input id="${escaparHTML(campoCrachaPrivadoId(serverId, 'rank'))}" type="text" placeholder="S / A / Elite" oninput="atualizarPreviewCrachaPrivadoAdmin('${escaparHTML(serverId)}')">
                        </label>
                        <label>
                            <span>Divisão</span>
                            <input id="${escaparHTML(campoCrachaPrivadoId(serverId, 'division'))}" type="text" placeholder="Alpha, Beta, Staff..." oninput="atualizarPreviewCrachaPrivadoAdmin('${escaparHTML(serverId)}')">
                        </label>
                        <label>
                            <span>Função</span>
                            <input id="${escaparHTML(campoCrachaPrivadoId(serverId, 'role'))}" type="text" placeholder="Líder, membro, suporte..." oninput="atualizarPreviewCrachaPrivadoAdmin('${escaparHTML(serverId)}')">
                        </label>
                        <label>
                            <span>Status</span>
                            <input id="${escaparHTML(campoCrachaPrivadoId(serverId, 'status'))}" type="text" placeholder="Ativo" oninput="atualizarPreviewCrachaPrivadoAdmin('${escaparHTML(serverId)}')">
                        </label>
                        <label>
                            <span>Nível</span>
                            <input id="${escaparHTML(campoCrachaPrivadoId(serverId, 'level'))}" type="number" min="0" value="0" oninput="atualizarPreviewCrachaPrivadoAdmin('${escaparHTML(serverId)}')">
                        </label>
                        <label>
                            <span>Pontos</span>
                            <input id="${escaparHTML(campoCrachaPrivadoId(serverId, 'points'))}" type="number" min="0" value="0" oninput="atualizarPreviewCrachaPrivadoAdmin('${escaparHTML(serverId)}')">
                        </label>
                        <label>
                            <span>Cor</span>
                            <input id="${escaparHTML(campoCrachaPrivadoId(serverId, 'color'))}" type="color" value="#35d8ff" oninput="atualizarPreviewCrachaPrivadoAdmin('${escaparHTML(serverId)}')">
                        </label>
                        <label class="admin-private-field-wide">
                            <span>Foto do membro</span>
                            <input id="${escaparHTML(campoCrachaPrivadoId(serverId, 'photoUrl'))}" type="url" placeholder="https://..." oninput="atualizarPreviewCrachaPrivadoAdmin('${escaparHTML(serverId)}')">
                        </label>
                        <label class="admin-private-field-wide">
                            <span>Tags / conquistas</span>
                            <input id="${escaparHTML(campoCrachaPrivadoId(serverId, 'tags'))}" type="text" placeholder="PvP, Staff, Evento, Top 1" oninput="atualizarPreviewCrachaPrivadoAdmin('${escaparHTML(serverId)}')">
                        </label>
                        <label class="admin-private-field-wide">
                            <span>Observações</span>
                            <textarea id="${escaparHTML(campoCrachaPrivadoId(serverId, 'notes'))}" rows="4" placeholder="Informações internas desse membro..." oninput="atualizarPreviewCrachaPrivadoAdmin('${escaparHTML(serverId)}')"></textarea>
                        </label>
                        ...
                        <p class="admin-private-status" id="admin-private-status-${escaparHTML(serverId)}"></p>
                    </form>

                    <aside class="admin-private-preview" id="admin-private-preview-${escaparHTML(serverId)}"></aside>
                </div>

                <div class="admin-private-list-head">
                    <strong>Crachás salvos</strong>
                    <button type="button" onclick="carregarCrachasPrivadosAdmin('${escaparHTML(serverId)}')">
                        <i class="ph ph-arrows-clockwise"></i>
                        Atualizar lista
                    </button>
                </div>
                <div class="admin-private-badge-list" id="admin-private-badges-${escaparHTML(serverId)}">
                    <span class="admin-member-empty">Carregando crachás...</span>
                </div>
            </div>
        </details>
    `;
...
function dadosFormularioCrachaPrivadoAdmin(serverId) {
    return {
        id: valorCampoCrachaPrivado(serverId, 'id'),
        memberName: valorCampoCrachaPrivado(serverId, 'memberName'),
        memberId: valorCampoCrachaPrivado(serverId, 'memberId'),
        badgeTitle: valorCampoCrachaPrivado(serverId, 'badgeTitle') || 'Crachá AMZ',
        rank: valorCampoCrachaPrivado(serverId, 'rank'),
        division: valorCampoCrachaPrivado(serverId, 'division'),
        role: valorCampoCrachaPrivado(serverId, 'role'),
        status: valorCampoCrachaPrivado(serverId, 'status') || 'Ativo',
        level: Number.parseInt(valorCampoCrachaPrivado(serverId, 'level') || '0', 10) || 0,
        points: Number.parseInt(valorCampoCrachaPrivado(serverId, 'points') || '0', 10) || 0,
        photoUrl: valorCampoCrachaPrivado(serverId, 'photoUrl'),
        color: valorCampoCrachaPrivado(serverId, 'color') || '#35d8ff',
        tags: textoParaLista(valorCampoCrachaPrivado(serverId, 'tags')),
        notes: valorCampoCrachaPrivado(serverId, 'notes')
    };
}

function renderizarPreviewCrachaPrivado(cracha = {}) {
    const nome = cracha.memberName || 'Nome do membro';
    const cor = cracha.color || '#35d8ff';
    const foto = cracha.photoUrl
        ? `<img src="${escaparHTML(cracha.photoUrl)}" alt="${escaparHTML(nome)}">`
        : `<span>${escaparHTML(obterIniciaisServidor(nome))}</span>`;
    const tags = Array.isArray(cracha.tags) && cracha.tags.length
        ? cracha.tags.map((tag) => `<em>${escaparHTML(tag)}</em>`).join('')
        : '<em>Sem tags</em>';

    return `
        <div class="admin-private-badge-preview" style="--badge-color: ${escaparHTML(cor)}">
            <div class="admin-private-badge-top">
                <div class="admin-private-badge-photo">${foto}</div>
                <div>
                    <span>${escaparHTML(cracha.badgeTitle || 'Crachá AMZ')}</span>
                    <strong>${escaparHTML(nome)}</strong>
                    <small>${escaparHTML(cracha.memberId ? `ID ${cracha.memberId}` : 'ID não definido')}</small>
                </div>
            </div>
            <div class="admin-private-badge-stats">
                <span>Rank <strong>${escaparHTML(cracha.rank || '--')}</strong></span>
                <span>Divisão <strong>${escaparHTML(cracha.division || '--')}</strong></span>
                <span>Nível <strong>${escaparHTML(cracha.level ?? 0)}</strong></span>
                <span>Pontos <strong>${escaparHTML(cracha.points ?? 0)}</strong></span>
            </div>
            <div class="admin-private-badge-meta">
                <span>${escaparHTML(cracha.role || 'Função não definida')}</span>
                <span>${escaparHTML(cracha.status || 'Ativo')}</span>
            </div>
            <p>${escaparHTML(cracha.notes || 'Observações internas aparecem aqui.')}</p>
            <div class="admin-private-badge-tags">${tags}</div>
        </div>
    `;
}

function atualizarPreviewCrachaPrivadoAdmin(serverId) {
    const preview = document.getElementById(`admin-private-preview-${serverId}`);
    if (!preview) return;
    preview.innerHTML = renderizarPreviewCrachaPrivado(dadosFormularioCrachaPrivadoAdmin(serverId));
}

function mostrarStatusCrachaPrivadoAdmin(serverId, mensagem, sucesso = false) {
    const status = document.getElementById(`admin-private-status-${serverId}`);
    if (!status) return;
    status.textContent = mensagem || '';
    status.classList.toggle('success', Boolean(sucesso));
}

function limparFormularioCrachaPrivadoAdmin(serverId) {
    ['id', 'memberName', 'memberId', 'rank', 'division', 'role', 'photoUrl', 'tags', 'notes'].forEach((campo) => {
        setValorCampoCrachaPrivado(serverId, campo, '');
    });
    setValorCampoCrachaPrivado(serverId, 'badgeTitle', 'Crachá AMZ');
    setValorCampoCrachaPrivado(serverId, 'status', 'Ativo');
    setValorCampoCrachaPrivado(serverId, 'level', '0');
    setValorCampoCrachaPrivado(serverId, 'points', '0');
    setValorCampoCrachaPrivado(serverId, 'color', '#35d8ff');
    mostrarStatusCrachaPrivadoAdmin(serverId, '');
    atualizarPreviewCrachaPrivadoAdmin(serverId);
}

function renderizarItemCrachaPrivadoAdmin(serverId, cracha) {
    const foto = cracha.photoUrl
        ? `<img src="${escaparHTML(cracha.photoUrl)}" alt="${escaparHTML(cracha.memberName || 'membro')}">`
        : `<span>${escaparHTML(obterIniciaisServidor(cracha.memberName || 'AMZ'))}</span>`;
    const tags = Array.isArray(cracha.tags) && cracha.tags.length
        ? cracha.tags.slice(0, 4).map((tag) => `<em>${escaparHTML(tag)}</em>`).join('')
        : '<em>Sem tags</em>';

    return `
        <article class="admin-private-badge-item" style="--badge-color: ${escaparHTML(cracha.color || '#35d8ff')}">
            <div class="admin-private-badge-mini">${foto}</div>
            <div>
                <strong>${escaparHTML(cracha.memberName || cracha.memberId || 'Membro')}</strong>
                <span>${escaparHTML(cracha.badgeTitle || 'Crachá AMZ')} / Rank ${escaparHTML(cracha.rank || '--')} / ${escaparHTML(cracha.division || 'Sem divisão')}</span>
                <small>${escaparHTML(cracha.role || 'Sem função')} · Nível ${escaparHTML(cracha.level ?? 0)} · ${escaparHTML(cracha.status || 'Ativo')}</small>
                <div>${tags}</div>
            </div>
            <div class="admin-private-badge-buttons">
                <button type="button" onclick="editarCrachaPrivadoAdmin('${escaparHTML(serverId)}', '${escaparHTML(cracha.id)}')">Editar</button>
                <button type="button" class="danger" onclick="removerCrachaPrivadoAdmin('${escaparHTML(serverId)}', '${escaparHTML(cracha.id)}')">Remover</button>
            </div>
        </article>
    `;
}

function renderizarListaCrachasPrivadosAdmin(serverId, crachas = []) {
    const lista = document.getElementById(`admin-private-badges-${serverId}`);
    if (!lista) return;

    adminPrivateBadgesCache[serverId] = Array.isArray(crachas) ? crachas : [];

    if (!adminPrivateBadgesCache[serverId].length) {
        lista.innerHTML = '<span class="admin-member-empty">Nenhum crachá privado criado ainda.</span>';
        return;
    }

    lista.innerHTML = adminPrivateBadgesCache[serverId]
        .map((cracha) => renderizarItemCrachaPrivadoAdmin(serverId, cracha))
        .join('');
}

async function carregarCrachasPrivadosAdmin(serverId, opcoes = {}) {
    const token = obterAdminToken();
    const lista = document.getElementById(`admin-private-badges-${serverId}`);

    if (!token || !lista) return;
    if (!opcoes.silencioso) lista.innerHTML = '<span class="admin-member-empty">Carregando crachás...</span>';

    try {
        const response = await fetch(`${API_URL}/api/admin/servidores/${encodeURIComponent(serverId)}/privado/crachas`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        const dados = await lerJsonResposta(response);

        if (response.ok && dados.status === 'sucesso') {
            renderizarListaCrachasPrivadosAdmin(serverId, dados.crachas || []);
            return;
        }

        lista.innerHTML = `<span class="admin-member-empty">${escaparHTML(dados.mensagem || dados.erro || 'Não foi possível carregar os crachás.')}</span>`;
    } catch (erro) {
        console.error('Erro ao carregar crachás privados:', erro);
        lista.innerHTML = '<span class="admin-member-empty">Erro ao conectar na API.</span>';
    }
}

async function salvarCrachaPrivadoAdmin(evento, serverId) {
    evento.preventDefault();
    const token = obterAdminToken();
    if (!token) return;

    const dados = dadosFormularioCrachaPrivadoAdmin(serverId);
    if (!dados.memberName && !dados.memberId) {
        mostrarStatusCrachaPrivadoAdmin(serverId, 'Informe o nome ou ID Discord do membro.');
        return;
    }

    mostrarStatusCrachaPrivadoAdmin(serverId, 'Salvando crachá...');
    ...
    try {
        ...
        const resposta = await lerJsonResposta(response);

        if (response.ok && resposta.status === 'sucesso') {
            renderizarListaCrachasPrivadosAdmin(serverId, resposta.crachas || []);
            setValorCampoCrachaPrivado(serverId, 'id', resposta.cracha?.id || dados.id || '');
            mostrarStatusCrachaPrivadoAdmin(serverId, resposta.mensagem || 'Crachá salvo.', true);
            atualizarPreviewCrachaPrivadoAdmin(serverId);
            return;
        }

        mostrarStatusCrachaPrivadoAdmin(serverId, resposta.mensagem || resposta.erro || 'Não foi possível salvar.');
    } catch (erro) {
        console.error('Erro ao salvar crachá privado:', erro);
        mostrarStatusCrachaPrivadoAdmin(serverId, 'Erro ao conectar na API.');
    }
}

function editarCrachaPrivadoAdmin(serverId, crachaId) {
    const cracha = (adminPrivateBadgesCache[serverId] || []).find((item) => item.id === crachaId);
    if (!cracha) return;

    setValorCampoCrachaPrivado(serverId, 'id', cracha.id);
    setValorCampoCrachaPrivado(serverId, 'memberName', cracha.memberName);
    setValorCampoCrachaPrivado(serverId, 'memberId', cracha.memberId);
    setValorCampoCrachaPrivado(serverId, 'badgeTitle', cracha.badgeTitle);
    setValorCampoCrachaPrivado(serverId, 'rank', cracha.rank);
    setValorCampoCrachaPrivado(serverId, 'division', cracha.division);
    setValorCampoCrachaPrivado(serverId, 'role', cracha.role);
    setValorCampoCrachaPrivado(serverId, 'status', cracha.status);
    setValorCampoCrachaPrivado(serverId, 'level', cracha.level);
    setValorCampoCrachaPrivado(serverId, 'points', cracha.points);
    setValorCampoCrachaPrivado(serverId, 'photoUrl', cracha.photoUrl);
    setValorCampoCrachaPrivado(serverId, 'color', cracha.color || '#35d8ff');
    setValorCampoCrachaPrivado(serverId, 'tags', Array.isArray(cracha.tags) ? cracha.tags.join(', ') : '');
    setValorCampoCrachaPrivado(serverId, 'notes', cracha.notes);
    mostrarStatusCrachaPrivadoAdmin(serverId, 'Editando crachá existente.', true);
    atualizarPreviewCrachaPrivadoAdmin(serverId);
    document.getElementById(campoCrachaPrivadoId(serverId, 'memberName'))?.scrollIntoView({ behavior: 'smooth', block: 'center' });
}

async function removerCrachaPrivadoAdmin(serverId, crachaId) {
    const token = obterAdminToken();
    if (!token) return;

    const cracha = (adminPrivateBadgesCache[serverId] || []).find((item) => item.id === crachaId);
    const nome = cracha?.memberName || crachaId;

    if (!window.confirm(`Remover o crachá privado de "${nome}"?`)) return;

    try {
        const response = await fetch(`${API_URL}/api/admin/servidores/${encodeURIComponent(serverId)}/privado/crachas/${encodeURIComponent(crachaId)}`, {
            method: 'DELETE',
            headers: { 'Authorization': `Bearer ${token}` }
        });
        const dados = await lerJsonResposta(response);
        ...
        if (response.ok && dados.status === 'sucesso') {
            renderizarListaCrachasPrivadosAdmin(serverId, dados.crachas || []);
            mostrarStatusCrachaPrivadoAdmin(serverId, dados.mensagem || 'Crachá removido.', true);
            if (valorCampoCrachaPrivado(serverId, 'id') === crachaId) limparFormularioCrachaPrivadoAdmin(serverId);
            return;
        }

        alert(dados.mensagem || dados.erro || 'Não foi possível remover o crachá.');
    } catch (erro) {
        console.error('Erro ao remover crachá privado:', erro);
        alert('Erro ao conectar na API.');
    }
}
