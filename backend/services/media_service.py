...
class MediaService:
    ...
    def imagem_para_gif(self, input_path, output_path):
        with Image.open(input_path) as imagem:
            if imagem.width * imagem.height > self.limits.max_image_pixels:
                raise MediaError("Imagem grande demais. Reduza para ate 12 megapixels.")
    ...
    def video_para_gif(self, input_path, output_path):
        escala = (
            f"scale=w='min({self.limits.max_width},iw)':h='min({self.limits.max_width},ih)'"
            ":force_original_aspect_ratio=decrease:flags=lanczos"
        )
        filtro = (
            f"fps={self.limits.gif_fps},{escala},"
